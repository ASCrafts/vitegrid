...
# ---------------------------------------------------------------------------
try:
    import honey_runtime  # type: ignore
    HAS_HONEY = True
except ImportError:
    HAS_HONEY = False

class DeterministicMasker:
    """Stateful Bidirectional Masking Pipeline for Algorithmic Interdiction.
    
    Uses high-performance Zig/Honey runtime if available, gracefully falling
    back to standard Python DFA regex execution.
    """
    
    def __init__(self):
        global HAS_HONEY
        self.state_dict: Dict[str, str] = {}
        self.counters = {"URL": 0, "ID": 0, "ADJ": 0}
        
        if HAS_HONEY:
            try:
                self.honey_module = honey_runtime.load_script("interdiction.honey")
            except Exception as e:
                print(f"[Interdiction Warning] Failed to load Honey script. Falling back to Python. Error: {e}")
                HAS_HONEY = False
                
        if not HAS_HONEY:
            self.patterns = [
                # 1. Network Topologies and URLs
                ("URL", re.compile(r'(?:https?:\/\/)?(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(?:\/[^\s]*)?')),
                # 2. Hyphenated Administrator IDs
                ("ID", re.compile(r'\b[a-zA-Z0-9]+-[a-zA-Z0-9-]+\b')),
                # 3. Hyphenated Adjectives
                ("ADJ", re.compile(r'\b[a-z]+-[a-z]+\b'))
            ]

    def mask(self, text: str) -> str:
        """O(N) Extraction and Placeholder Injection."""
        if not text:
            return text
            
        if HAS_HONEY and hasattr(self, 'honey_module'):
            result = self.honey_module.mask_payload(text)
            self.state_dict.update(result["ledger"])
        ...
        for entity_type, pattern in self.patterns:
            ...
            for match in matches:
                # Prevent recursive masking
                if match.startswith("__") and match.endswith("__"):
                    continue
                
                self.counters[entity_type] += 1
                placeholder = f"__{entity_type}_ENTITY_{self.counters[entity_type]:03d}__"
                self.state_dict[placeholder] = match
                
                # Replace vulnerable string with tokenization-resistant placeholder
                masked_text = masked_text.replace(match, placeholder)
        return masked_text

    def demask(self, text: str) -> str:
        """O(N) Scanning pass with O(1) Dictionary Lookup Substitution."""
        if not text or not self.state_dict:
            return text
            
        if HAS_HONEY and hasattr(self, 'honey_module'):
            return self.honey_module.demask_payload(text, self.state_dict)
            
        for placeholder, original in self.state_dict.items():
...
class StyleTokens(BaseModel):
    ...
    border_style: Literal["none", "solid", "dashed", "dotted", "double"] = Field(  # type: ignore[assignment]
    ...
    line_alignment: Literal["top", "bottom", "left", "right", "all", "none"] = Field(  # type: ignore[assignment]
...
def _parse_response(response: Any, schema: type[BaseModel], masker: Optional[DeterministicMasker] = None) -> BaseModel:
    """Parses LLM output, applying O(1) de-masking before Pydantic validation."""
    ...
    # --- INTERDICTION: Execute O(1) Dictionary Lookup Substitution ---
    if masker:
        text = masker.demask(text)
    # ------------------------------------------------------------------
...
def agent3_generate_from_prompt(user_goal: str, patch_directive: str | None = None) -> DocumentLayout:
    ...
    # 1. Mask incoming fragile strings
    masked_goal = masker.mask(user_goal)
    
    # 2. Inject operational constraints to the attention mechanism
    constrained_prompt = _AGENT3_PROMPT + "\n\nCRITICAL CONSTRAINT: You must formally recognize and replicate any string matching the __.*__ pattern (e.g., __URL_ENTITY_001__) with absolute zero alteration."
    
    parts: list[str] = [constrained_prompt, f"User goal:\n{masked_goal}"]
    if patch_directive:
        masked_patch = masker.mask(patch_directive)
        parts.append(f"Auditor patch instructions to address:\n{masked_patch}")
        
    config = _json_config(DocumentLayout)
    
    # 3. Force Thermodynamic Greedy Decoding (Limit as T approaches 0)
    ...
    # 4. Pass the masker for bidirectional de-masking
    return _parse_response(response, DocumentLayout, masker=masker)  # type: ignore[return-value]
...
def _normalize_margins(raw: dict[str, float] | dict[str, int] | None) -> dict[str, float]:
    raw = raw or {}
    aliases = {
    ...
    for canonical, names in aliases.items():
        value = next((raw[n] for n in names if n in raw and raw[n] is not None), 72.0)
        try:
...
def auto_layout(layout: DocumentLayout) -> DocumentLayout:
    layout.margin_px = _normalize_margins(layout.margin_px)  # type: ignore[assignment]
...
def _dominant_style(span_indices: list[int], spans: list[Any]) -> StyleTokens:
    ...
    return StyleTokens(
        font_family=max(set(fonts), key=fonts.count) if fonts else "Arial",
        ...
        color_hex=max(set(colors), key=colors.count) if colors else "000000",
    )
            ...
            for idx in missing:
                ...
                blocks.append(
                    DocumentBlock(
                        ...
                        bbox=bbox,
                        style=StyleTokens(
                            font_family=span.font or "Arial",
                            font_size_pt=span.size_pt or 11.0,
                            font_weight="bold" if span.bold else "normal",
                            color_hex=span.color_hex or "000000",
                        ),
                    )
                )
                counter += 1
...
def import_from_classified_blocks(
    ...
    for i, c in enumerate(classified):
        ...
        style = StyleTokens(
            ...
            align=c.align,
            border_visible=True if c.block_type == "table" else False,
        )
        table_cells = None
...
def import_from_docx_blocks(docx_blocks: list[Any]) -> tuple[DocumentLayout, AuditReport]:
    ...
    for i, b in enumerate(docx_blocks):
        ...
        style = StyleTokens(
            ...
            border_visible=True if b.type == "table" else False,
        )
...
def agent6_chat(layout: DocumentLayout, history: list[ChatTurn], user_message: str) -> ChatResponse:
    masker = DeterministicMasker()
    masked_user_message = masker.mask(user_message)
    
    constrained_prompt = _AGENT6_PROMPT + "\n\nCRITICAL CONSTRAINT: You must formally recognize and replicate any string matching the __.*__ pattern (e.g., __URL_ENTITY_001__) with absolute zero alteration."
    ...
    response = _call_with_retry(
        _core_client(),
        model=_generation_model(),
        contents=[constrained_prompt, payload],
        config=config,
    )
    result = _parse_response(response, ChatResponse, masker=masker)  # type: ignore[assignment]
        ...
        try:
            ...
            try:
                ...
                for page_gt in doc_gt:
                    gt_text += str(page_gt.get_text("text"))
                doc_gt.close()
            except Exception:
                gt_text = ""
