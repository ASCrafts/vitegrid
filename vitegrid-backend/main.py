...
def upload_image(
    ...
    return ImageUploadResponse(
        ...
        local_path=cast(str, asset.local_path),
        width_px=width,
        height_px=height,
    )
...
def list_templates(db: Session = Depends(get_db)) -> list[TemplateSummary]:
    ...
    return [
        TemplateSummary(
            ...
            lock_tier=cast(int, row.lock_tier),
            thumbnail_path=cast(str, row.thumbnail_path) if row.thumbnail_path is not None else None,
...
def get_template(template_id: int, db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    return {
        ...
        "layout": json.loads(cast(str, tpl.layout_json)),
...
def update_template(
    ...
    if req.name is not None:
        tpl.name = req.name  # type: ignore[assignment]
    if req.layout is not None:
        tpl.layout_json = json.dumps(req.layout)  # type: ignore[assignment]
    if req.lock_tier is not None:
        if req.lock_tier not in {1, 2, 3}:
            raise HTTPException(status_code=400, detail="lock_tier must be 1, 2, or 3")
        tpl.lock_tier = req.lock_tier  # type: ignore[assignment]
    db.commit()
    return {"id": tpl.id}
